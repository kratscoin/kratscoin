...
const char * const KRATSCOIN_CONF_FILENAME = "kratscoin.conf";
const char * const KRATSCOIN_PID_FILENAME = "kratscoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "kratscoind -foo=bar
...
#else
    const char* pszModule = "kratscoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Kratscoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Kratscoin
    // Mac: ~/Library/Application Support/Kratscoin
    // Unix: ~/.kratscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kratscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kratscoin";
#else
    // Unix
    return pathRet / ".kratscoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", KRATSCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", KRATSCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Kratscoin Core") == std::string::npos) {
