...
###   kratscoincore.org and kratscoin.org and compares them.
...
WORKINGDIR="/tmp/kratscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://kratscoincore.org"
HOST2="https://kratscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="kratscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kratscoin-',
...
if [ $? -ne 0 ]; then
   echo "kratscoin.org failed to provide signature file, but kratscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "kratscoin.org and kratscoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Kratscoin Core binary release signing key installed?"
   fi
